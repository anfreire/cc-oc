...
- **`/oc:wait` is invoked through Monitor with `persistent: true`.** The old guidance — spawn's `Get the answer (blocks; run_in_background to be pinged when ready)` and wait.md's `Usually run in the background … run it directly when you want to block on it now` — led with the blocking framing and left backgrounding as an optional parenthetical, so the foreground path got taken. A foreground `Bash` call is capped at 120s by default and 600s at most, under what an opencode session routinely takes: blocking didn't merely serialize the agent, past the cap the call died and the answer never came back at all. `persistent: true` has no timeout and delivers the answer as a notification when the session ends.
- **The Monitor command redirects stderr (`2>&1`).** Monitor raises notifications for stdout only, and `wait` reports errors and cancellations on stderr. Without the redirect a failed or cancelled session ends the watch with exit 1 and no message — indistinguishable from silence, which is the one shape a watch must never fail into.
- **`/oc:spawn` prints `/oc:wait <id>` instead of a raw `node … wait <id>` invocation.** `wait` was the only one of the four follow-ups not already printed as a slash command, and the raw form forced the CLI to explain backgrounding itself — naming a Claude Code tool it can't guarantee exists, in a second place bound to drift from `commands/wait.md`. The how now lives in `wait.md` alone.
- **The `log:` line is gone from a successful spawn, and conditional on a failed one.** Printing `<sid>.ndjson` on every successful start put the path in context long before anything went wrong, which is where the read-the-raw-log habit came from; the follow-up block already lists `/oc:debug`. On failure the line is now `debug:    /oc:debug <id>` when a session id exists — the `error-event` path records a ledger entry, so the trace is reachable — and stays `log:      <path>` for `timeout`, `no-events`, and `spawn-error`, which register nothing and whose temp log is the only artifact, often the only place opencode's pre-JSON stderr landed.
- **Docs.** `commands/wait.md` rewritten around the Monitor invocation and its `allowed-tools` extended with `Monitor` (pre-approval only — the field grants permission for the invoking turn and does not restrict what is callable, so it removes a prompt rather than enabling anything). `README.md`'s `/oc:wait` section and `cmdWait`'s JSDoc updated to match.

## 0.8.0 — 2026-07-02

**`/oc:tail` replaced by `/oc:debug` — one flagless command that prints a session's status and full event trace.** Field data since the 0.7.0 wait/tail split: `wait` is used constantly, `tail` was never used once — not the peek form, not `--follow`. Whenever the driving agent actually needed to inspect a session (progress, failures) it read the raw `<sid>.ndjson` log and parsed the events itself, because `tail`'s default (`--events 1`) showed too little and picking the right flag combination wasn't worth it. `debug` is the plugin's own parsing pipeline pointed at that job: always the whole story, no modes to choose. `/oc:wait` is unchanged.

### Changed

- **New `debug` subcommand (`/oc:debug <id>`), replacing `tail`.** Prints a `session:` / `status:` / `activity:` header followed by every renderable event with session-relative timestamps. One output shape for every session state — a `running` session shows its trace so far (`running` + stale `activity` = the usage-limit stall from `reference/recovery.md`), a finished one the whole run, and a missing or empty log renders `(no events)` instead of dying. No `--follow` (a backgrounded `wait` already covers "tell me when it's done"), no `--events` (the full trace is the point).
- **`lib/tail.mjs` → `lib/log.mjs`.** `followLog` (the 15-min polling loop) and `readDigest` (render-then-slice-last-N plus follow-mode offset bookkeeping) are gone; a new `renderLog(logFile)` returns the full rendered trace, `""` for a missing/unreadable/event-less log. `renderEvent` and `extractErrorMessage` are module-internal now — the exported API is `renderLog`, `readLogState`, `isPidAlive`.
- **Multi-line `model:` / `thinking:` bodies start on the header line**, with continuation lines indented two spaces, instead of pushing the whole body under a bare `[ts] model:` header. Flush-left in a trace now always means a new event — nothing a model prints can be mistaken for an event header. Bodies are otherwise verbatim; one shape whether the text is one line or fifty.
- **`/oc:spawn` output** — the "Watch or inspect events" block is now "Inspect or manage": `/oc:debug` / `/oc:cancel` / `/oc:sessions`.
- **`wait`'s no-model-text diagnostic** points at `/oc:debug` (was `/oc:tail`).
- **Startup probe budget: 90s → 60s** (`PROBE_MS` in `spawn.mjs`). Still triple the cold-start window that motivated 0.7.3's 20s → 90s raise, and — with the probe no longer settling on error events — a minute bounds both "first event never came" and "error never recovered". Docs that quote the budget updated.

### Fixed

- **An error opencode recovered from no longer fails the session (model fallbacks).** cc-oc treated the first error event as the verdict: the startup probe killed a still-alive opencode the moment one appeared — exactly when a configured fallback model was about to rescue the run — and `wait`/reconcile stamped `error` even when the session had recovered and finished, discarding the real answer. Failure is now judged by the log's **last word**: an error event followed by model work (`step_start` / `text` / `tool_use` / `reasoning`) is transient — still shown verbatim in `/oc:debug`, but not the status. The probe never settles on an error while opencode is alive; a fatal startup error still surfaces inline when opencode exits with it unrecovered, and a probe timeout now reports the pending error message instead of the generic "did not produce any event" line. `LogState.errorMessage` carries the *last unrecovered* error rather than the first one seen. `step_finish` deliberately doesn't count as recovery (opencode could emit it while unwinding a failed step), nor do `stderr` noise or unknown event kinds.
- **`oc.mjs` exits quietly when its output pipe closes early** (`oc.mjs debug <id> | head`) instead of crashing with an unhandled `EPIPE` stack trace — relevant now that `debug` dumps full traces worth piping through `head` / `grep`.

### Docs

- **Command skills rewritten minimal.** Dropped the per-file `## Arguments` boilerplate, the `## Related commands` blocks (the skill list already carries every command's description), and spawn's verbatim output examples (spawn's real output prints the follow-up commands itself). Kept what the driving agent can't infer: the spawn parse-and-compose rule, the flag vocabulary, exit-code/background semantics on `wait`, and `debug`'s trace-reading notes (indent grammar, recovered errors, the stall signature). Softened restrictive phrasing ("never read the raw log" → "no need to") so wording doesn't fence off legitimate uses, and spawn's flags table became a plain list — a two-column table is a definition list in costume, and the list reads the same to a model without the pipe-and-padding overhead (the README keeps its table; that page is rendered for humans).
- `commands/tail.md` → `commands/debug.md`; `README.md`, `commands/{spawn,sessions}.md`, `reference/recovery.md`, and both manifest descriptions updated for the new surface. While in there: the README's "20-second probe" line — stale since 0.7.3 — now quotes the real budget, and the statuses documented for `/oc:sessions` include `cancelled` with the pid-liveness semantics from 0.5.0/0.7.2 (the old text still described terminal-event detection and cancel-as-`done`).
